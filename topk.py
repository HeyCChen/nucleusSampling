...
def topk(
    ...
    if min_score is not None:
        ...
        dense_x = x.new_full((batch_size * max_num_nodes, ), 600000000.0)
        dense_x[index] = x
        dense_x = dense_x.view(batch_size, max_num_nodes)

        _, perm = dense_x.sort(dim=-1, descending=False)
        ...
        if isinstance(ratio, int) and (k == ratio).all():
            ...
            index = torch.cat([
                torch.arange(k[i], device=x.device) +
                int(0.25*num_nodes[i]) + i * max_num_nodes
